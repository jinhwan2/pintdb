...
public class SingleThreadTests extends IntegrationTestConfig {
  /*
   * test scenario 
   * 1. Data order 
   *    String -> HashMap -> List
  ...
  public void testA() {
    ...
    IntStream.range(0, TEST).parallel().forEach(i -> {
      requestForm.setKey(key + "_" + i);
  ...
  public void testB() {
    ...
    IntStream.range(0, TEST).parallel().forEach(i -> {
      databaseController.getString(key + "_" + i);
    });
  ...
  public void testD() {
    ...
    IntStream.range(0, TEST).forEach(i -> {
      databaseController.getHashMap(key + "_" + i);
    });
  ...
  public void testF() {
    ...
    IntStream.range(0, TEST).forEach(i -> {
      databaseController.getList(key + "_" + i);
    });
